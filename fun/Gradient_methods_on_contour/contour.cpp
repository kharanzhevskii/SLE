...
void richardson(const CSR<double>& csr, const std::vector<double>& b, const std::vector<double>& x0, const double percision, const double t){
    ...
    std::ofstream timer, counter1, counter2;
    std::string timerstr = std::to_string(t) + "_rich_timer.txt";
    std::string counterstr1 = std::to_string(t) + "_rich_lmin.txt";
    std::string counterstr2 = std::to_string(t) + "_rich_lmax.txt";
    timer.open(timerstr);
    counter1.open(counterstr1);
    counter2.open(counterstr2);
    ...
    counter1 << x[0] << std::endl;
    counter2 << x[3] << std::endl;
...
void chebyshev(const CSR<double>& csr, const std::vector<double>& b, const std::vector<double>& x0, const double lambda_max, const double lambda_min, unsigned long int r_, const double percision){
    std::cout << "started" << std::endl;
    std::ofstream timer, counter1, counter2;
    std::string timerstr = std::to_string(r_) + "_cheb_timer.txt";
    std::string counterstr1 = std::to_string(r_) + "_cheb_lmin.txt";
    std::string counterstr2 = std::to_string(r_) + "_cheb_lmax.txt";
    timer.open(timerstr);
    counter1.open(counterstr1);
    counter2.open(counterstr2);
    std::chrono::high_resolution_clock::time_point begin, end;
...
void FGD(const CSR<double>& csr, const std::vector<double>& b, const std::vector<double>& x0, const double percision){
    std::cout << "started" << std::endl;
    std::ofstream timer, counter1, counter2;
    std::string timerstr = "fgd_timer.txt";
    std::string counterstr1 = "fgd_lmin.txt";
    std::string counterstr2 = "fgd_lmax.txt";
    timer.open(timerstr);
    counter1.open(counterstr1);
    counter2.open(counterstr2);
    std::chrono::high_resolution_clock::time_point begin, end;
...
void CG(const CSR<double>& csr, const std::vector<double>& b, const std::vector<double>& x0, const double percision = 1e-8){
    std::cout << "started" << std::endl;
    std::ofstream timer, counter1, counter2;
    std::string timerstr = "cg_timer.txt";
    std::string counterstr1 = "cg_lmin.txt";
    std::string counterstr2 = "cg_lmax.txt";
    timer.open(timerstr);
    counter1.open(counterstr1);
    counter2.open(counterstr2);
    std::chrono::high_resolution_clock::time_point begin, end;
...
int main(){
    std::vector<double> val{3, 3, 4, 4.5};
    std::vector<unsigned long int> col{0, 1, 2, 3};
    std::vector<unsigned long int> row{0, 1, 2, 3, 4};
    CSR<double> csr(val, col, row);
    std::vector<double> b{5, 5, 5, 5}, x0{0, 0, 0, 0};
    double percision = 1e-12, lambda_min, lambda_max;
    long unsigned int r_= 8;

    lambda_min = 3;
    lambda_max = 4.5;
    const double t2 = 2 / (lambda_min + lambda_max);
    const double t1 = 0.9 * 2 / lambda_max;

    FGD(csr, b, x0, percision);
    CG(csr, b, x0, percision);
    chebyshev(csr, b, x0, lambda_max, lambda_min, r_, percision);
    richardson(csr, b, x0, percision, t1);
    richardson(csr, b, x0, percision, t2);
